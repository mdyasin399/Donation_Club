<?php 
  ...
  $ques = mysqli_query($db,"SELECT q_id, Status, question_detail, img, ques_points, username, Subject_Name, created_at, question_img, id  FROM questions q
       ...
       JOIN subjects sub ON q.subject_code=sub.s_code WHERE q.q_id='$qu_id'")or die(mysqli_error());
...
<?php
       ...
       JOIN students st ON s.student_id=st.id WHERE s.ques_id='$qu_id'")or die(mysqli_error());
        ...
        $interval2 = $now2->diff($p2);
			...
			$sol_count=mysqli_query($db,"SELECT COUNT(answer) FROM solutions where student_id='$sol_id'")or die(mysqli_error());
  ...
  ?>
  	<div class="ans-view">
	  	<div class="qsn">
		  	...
		  	<div class="qsn-main">
		  		<p>Interest Rate(%): <span><?php echo $sol_row['answer']; ?></span></p>
